...
exports.Swig = function (opts) {
   ...
   * @param {object}    config  some config to filter
   * @param {boolean}   config.dontOverride  Can filter be overridden
   * @return {undefined}
   */
  this.setFilter = function (name, method, config = {}) {
    if (typeof method !== 'function') {
      throw new Error('Filter "' + name + '" is not a valid function.')
    }
    const dontOverride = config.dontOverride || false
    if (dontOverride) {
      mustFilters[name] = method
    }
    filters[name] = dontOverride && mustFilters[name] ? mustFilters[name] : method
  }
